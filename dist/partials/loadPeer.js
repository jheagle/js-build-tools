...
exports.requirePeer = exports.importPeer = exports.importModule = void 0;
 ...
 */
const importModule = specifier => dynamicImport(specifier);
exports.importModule = importModule;
const isMissing = (error, name) => error.code === 'MODULE_NOT_FOUND' && error.message.includes(`'${name}'`);
const notInstalled = (name, {
  task,
  installHint
}) => new Error(`The ${task} task needs the optional peer dependency ${name}, which is not installed. ${installHint}`);
...
const requirePeer = (name, {
  ...
  try {
    return (0, _nodeModule.createRequire)(_nodePath.default.join(projectPath, 'package.json'))(name);
  } catch (error) {
 ...
 * @throws {Error} With install instructions when the package is not installed.
 */
exports.requirePeer = requirePeer;
const importPeer = async (name, {
  ...
  let resolved;
  try {
    resolved = (0, _nodeModule.createRequire)(_nodePath.default.join(projectPath, 'package.json')).resolve(name);
  } catch (error) {
    if (isMissing(error, name)) {
      throw notInstalled(name, {
        task,
  ...
  return importModule((0, _nodeUrl.pathToFileURL)(resolved).href);
};
exports.importPeer = importPeer;