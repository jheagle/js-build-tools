...
const printFileReport = (file, result) => {
  ...
  console.log(_nodePath.default.relative(file.cwd, file.path));
  result.messages.forEach(({
    line,
    column,
    message
  }) => console.log(`line ${line}:${column}\t${message}`));
...
const standardLint = ({
  fix = true
} = {}, loadStandard = () => importStandard().then(({
  default: standard
}) => standard)) => {
  const totals = {
    errors: 0,
  ...
  return _through.default.obj(function (file, enc, callback) {
    ...
    loadStandard().then(standard => standard.lintText(file.contents.toString(), {
      filename: file.path,
      cwd: file.cwd,
      fix
    })).then(results => {
