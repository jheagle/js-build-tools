...
var _promises2 = require("node:timers/promises");
const attempts = 4;
const exists = dirPath => (0, _promises.access)(dirPath).then(() => true, () => false);
...
const removeDirectory = async dirPath => {
  for (let attempt = 1; attempt <= attempts; attempt++) {
    ...
    if (attempt < attempts) {
      await (0, _promises2.setTimeout)(200 * attempt);
    }
  }
  throw new Error(`Unable to remove "${dirPath}" - it was recreated or is locked. Stop anything that has it open or is writing ` + 'into it (watchers, dev servers, editors, other build tasks) and try again.');
};
